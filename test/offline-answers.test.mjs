...
import { checkAnswer, unsupportedClaims, requiredCaveats } from '../js/agent-contract.js';
...
test('offline answers state calculated facts', () => {
  ...
  assert.match(offlineAnswer(s, { questionId: 'find-repeats', scope: null }).answer, /Bars 1, 2, 3, 4 and 6 are exactly the same/);
...
test('checkAnswer rejects references outside the shared bars and bar numbers without references', () => {
  ...
  assert.match(checkAnswer({ ...base, references: [] }, part).problems.join(), /add those bars to "references"/);
